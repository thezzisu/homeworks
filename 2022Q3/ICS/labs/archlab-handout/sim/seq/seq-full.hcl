...
# MIT Licence
# Search keyword "IADDQ" and "JM" for changes
# | Stage      | iaddq V, rB          | jm V, rB             |
# |------------|----------------------|----------------------|
...
# |------------|----------------------|----------------------|
# | Decode     | valB <- R[rB]        | valB <- R[rB]        |
# |------------|----------------------|----------------------|
# | Execuate   | valE <- valB + valC  | valE <- valB + valC  |
# |            | set CC               |                      |
# |------------|----------------------|----------------------|
# | Memory     |                      | valM <- M8[valE]     |
# |------------|----------------------|----------------------|
...
# IADDQ & JM : add corresponding icode to valid icodes
bool instr_valid = icode in 
	{ INOP, IHALT, IRRMOVQ, IIRMOVQ, IRMMOVQ, IMRMOVQ,
	       IOPQ, IJXX, ICALL, IRET, IPUSHQ, IPOPQ, IIADDQ, IJM };

# IADDQ & JM : regid byte required
# Does fetched instruction require a regid byte?
bool need_regids =
	icode in { IRRMOVQ, IOPQ, IPUSHQ, IPOPQ, 
		     IIRMOVQ, IRMMOVQ, IMRMOVQ, IIADDQ, IJM };

# IADDQ & JM : constant word required
...
# IADDQ & JM : value of rB is required
...
# IADDQ : the result would be write to rB
...
# IADDQ & JM : both of these instructions need to use ALU for add
...
# IADDQ & JM : set another ALU input
...
# JM : needs to load target address from memory
...
# JM : kinda like RET, use value loaded from memory as new PC
word new_pc = [
